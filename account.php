            ...
            <div class="card-body">
                ...
                ?>
                         <tr>
                                ...
                                <?php 
                                if($row['status'] == 1){
                                    echo '<td><span class="badge badge-primary ">Order Confirmed</span></td>';
                                }else if($row['status'] == 2){
                                    echo '<td><span class="badge badge-danger">Cancelled</span></td>';
                                }else if($row['status'] == 3){
                                    echo '<td><span class="badge badge-secondary">Order is being packed</span></td>';
                                }else if($row['status'] == 4){
                                    echo '<td><span class="badge badge-primary">Order is on the way</span></td>';
                                ...
                                <td>
                                    ...
                                    if($row['status'] == 0){
                                        ...
                                        echo ' <button type="button" class="btn btn-sm btn-success" onclick="handleReceive('.$row['order_id'].')">
                                        Receive
                                    </button>';
                                    }
                                     ?>
...
<script>
    function handleReceive(id) {
            start_load()
            $.ajax({
                url: 'admin/ajax.php?action=delivery_confirm',
                method: "POST",
                data: {
                    id,
                },
                success: function(resp) {
                    if (resp == 1) {
                        alert_toast("Order successfully Paid.", 'success')
                        setTimeout(() => {
                            location.reload();
                        }, 1000);
                    }
